...
public class FerroxWallJump : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        Debug.Log("Is the player grounded: " + GetComponent<FerroxJump>().isGrounded);
            ...
            {
                {
                    ...
                    GetComponent<Rigidbody2D>().velocity = new Vector2(speed / 2 * hit.normal.x, speed);
                    ...
                    // unpause input when grounded
                   if (GetComponent<FerroxJump>().isGrounded == true) {
                       GetComponent<FerroxMovement>().pauseInput = false;
                   }
                }
            }
        }
